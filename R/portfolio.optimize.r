"portfolio.optimize" <- function(object,ptf.mean=0.01,
                            ...
                            level=0.95,...)
{
  ## Object must be of class "ghypmv" or "mle.ghypmv"
  test.class.ghyp(object,case="ghypmv")
  
  risk.measure <- match.arg(risk.measure)
  
  if(risk.measure=="variance"){
    lin.sys <- rbind(object@variance,object@expected.value,rep(1,object@dimension))
    lin.sys <- cbind(lin.sys,c(object@expected.value,0,0),c(rep(1,object@dimension),0,0))
    b <- c(rep(0,object@dimension),ptf.mean,1)
    ptf.weights <- solve(lin.sys,b)[1:object@dimension]
    tmp.portfolio <- lin.transf(object,multiplier=t(ptf.weights))
    return(list(risk.measure=risk.measure,
               value= as.numeric(ptf.weights %*% object@variance %*% ptf.weights),
   ...
   objective <- function(opt.weights,object,minimize.func,ptf.mean,level){
     weight.2 <- (ptf.mean+ object@expected.value[1]*(sum(opt.weights)-1)-
                  sum(opt.weights*object@expected.value[3:object@dimension]))/
                  (object@expected.value[2]-object@expected.value[1])
     weight.1 <- 1-sum(opt.weights)-weight.2
     tmp.weights <- c(weight.1,weight.2,opt.weights)
     tmp.portfolio <- lin.transf(object,multiplier=t(tmp.weights))
   ...
   weight.2 <- (ptf.mean+ object@expected.value[1]*(sum(opt.ptf$par)-1)-
                sum(opt.ptf$par*object@expected.value[3:object@dimension]))/
                (object@expected.value[2]-object@expected.value[1])
   weight.1 <- 1-sum(opt.ptf$par)-weight.2
   tmp.weights <- c(weight.1,weight.2,opt.ptf$par)
   tmp.portfolio <- lin.transf(object,multiplier=t(tmp.weights))
   return(list(portfolio=tmp.portfolio,risk.measure=risk.measure,value=-opt.ptf$value,
               opt.weights=unname(tmp.weights),
